  ...
  const handleCustomGoogleLogin = async () => {
    if (window.google?.accounts?.id && googleClientId) {
      try {
        ...
        console.warn('Google prompt fallback trigger', e);
      }
    }

    const userEmail = prompt('Sign in with Google - Enter your Gmail address:', email || 'sanketbhende0@gmail.com');
    if (!userEmail || !userEmail.includes('@')) return;

    setLoading(true);
    setError('');
    try {
      const nameFromEmail = userEmail.split('@')[0].replace(/\./g, ' ');
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="flex flex-col items-center justify-center space-y-2">
                  <button
                    ...
                    disabled={loading}
                    className="w-full py-2.5 px-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/60 text-slate-800 dark:text-slate-100 text-xs font-black shadow-xs hover:shadow-md transition-all flex items-center justify-center gap-2.5 cursor-pointer"
                  >
                  ...
                  <div id="googleBtnContainer" className="hidden"></div>
                </div>
